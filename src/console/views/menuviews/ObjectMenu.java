...
public class ObjectMenu {
    ...
    public static void showFor(DataBase.viewFor view){
        
        while(true){                                                            //Runs the loop until the user choose to go back
            int result;
            Style.addLines(1);
            if (view == DataBase.viewFor.CREATE){ System.out.println(Style.yellow("     CREATE"));}// Choose what header to print among create,edit,or search.
            else if(view == DataBase.viewFor.EDIT){System.out.println(Style.yellow("     EDIT"));} 
            else if(view == DataBase.viewFor.SEARCH){System.out.println(Style.yellow("     SEARCH"));}
            ...
            switch(result){
                case 1:
                    if (view == DataBase.viewFor.CREATE){CreateStudent.show();}                             //Create Student procedure
                    else if(view == DataBase.viewFor.EDIT){EditStudent.show();}             //Edit Student procedure
                    else if(view == DataBase.viewFor.SEARCH){SearchInStudentsView.show();}                  //Search in database for Students procedure
                    else{ System.out.println("An unexpected error has occurred. ObjectMenu file, Line 40");}
                    break;

                case 2:
                    if (view == DataBase.viewFor.CREATE){CreateTrainer.show();}                             //Create Trainer procedure
                    else if(view == DataBase.viewFor.EDIT){{EditTrainer.show();}}             //Edit Trainer procedure
                    else if(view == DataBase.viewFor.SEARCH){SearchInTrainersView.show();}                  //Search in database for Trainers procedure
                    else{ System.out.println(Style.red("An unexpected error has occurred. ObjectMenu file, Line 47"));}
                    break;

                case 3:
                    if (view == DataBase.viewFor.CREATE){CreateCourse.show();}                              //Create Course procedure
                    else if(view == DataBase.viewFor.EDIT){EditCourse.show();}             //Edit Course procedure
                    else if(view == DataBase.viewFor.SEARCH){SearchInCoursesView.show();}                   //Search in database for Courses procedure
                    else{ System.out.println(Style.red("An unexpected error has occurred. ObjectMenu file, Line 54"));}
                    break;

                case 4:
                    if (view == DataBase.viewFor.CREATE){CreateAssignment.show();}                          //Create Assignment procedure
                    else if(view == DataBase.viewFor.EDIT){EditAssignment.show();}             //Edit Assignment procedure
                    else if(view == DataBase.viewFor.SEARCH){SearchInAssignmentsView.show();}               //Search in database for Assignments procedure
