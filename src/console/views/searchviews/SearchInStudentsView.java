...
import database.classes.DataConnectorTable;
...
public class SearchInStudentsView {
    ...
    private static void showStudents(){
        if(data.getStudents().size()>0){
            printStudents(data.getStudents());                                  //prints all the students
            Input.pressToGoBack();
        }
        else{
            Style.addLines(1);
    ...
    private static void showStudentDetails(){
        ...
        if(data.getStudents().size()>0){
            Student student = OtherViews.studentChoose.chooseOne(data.getStudents()); //The user chooses the student he wants
            ArrayList<Course> courses = data.getCoursesFromStudent(student);    //The program finds the student's courses
            DataConnectorTable<Course, Assignment> table = data.getAssignmentsFromStudent(student); //the program finds the assignments for every course
            ...
            Style.addUnderline();
            for(Course course:courses){ 
                assignments = table.getBWhenA(course);                          //finds the course's assignments,for each course and prints them
                for(Assignment assignment:assignments){
                    if(data.doesExistStudentGradeForAssignment(student, course, assignment)){
                        StudentGrade sT = data.getStudentGradeForAssignment(student, course, assignment);
    ...
    private static void showStudentsToDeliverAssignment(){
        if(data.getStudents().size()>0){                                        //Checks if there are Students
            if(data.getAssignments().size()>0){                                 //Checks if there are Assignments
                Style.addLines(1);
                ...
                ArrayList<Assignment> assignmentsForDeliver = data.AssignmentsMustDelivered(date);
                if(assignmentsForDeliver.size()>0){                             //Checks if there are assignments in that week
                    for(Assignment assignment : assignmentsForDeliver){
                        if(data.getCoursesFromAssignment(assignment).size()>0){ //Checks if there are courses in that assignment
                            for (Course course : data.getCoursesFromAssignment(assignment)){
                                ...
                                Style.addUnderline();
                                if(data.getStudentsFromCourse(course).size()>0){//checks if there are students in that course.
                                    int counter = 1;
                                    for(Student student: data.getStudentsFromCourse(course)){
                                        
                                        if(data.doesExistStudentGradeForAssignment(student, course, assignment)){
                                            StudentGrade studentGrade = data.getStudentGradeForAssignment(student,course,assignment);
