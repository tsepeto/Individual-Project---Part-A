...
public class SearchInAssignmentsView {
    ...
    private static void showAssignments(){
        if(data.getAssignments().size()>0){
            printAssignments(data.getAssignments());
    ...
    private static void showAssignmentDetails(){
         if(data.getAssignments().size()>0){
            Assignment result = OtherViews.assignmentChoose.chooseOne(data.getAssignments());//The user chooses assignment
            ArrayList<Course> courses = data.getCoursesFromAssignment(result);  //Gets the assignment's courses
