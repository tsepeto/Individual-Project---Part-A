...
public class EditStudent {
    ...
    private static void chooseEditor(){
        ...
        Student chosenStudent = OtherViews.studentChoose.chooseOne(data.getStudents()); // Lets the user to choose one student
    ...
    private static void rateStudentGrade(Student student){
        ...
        if(data.getCoursesFromStudent(student).size()> 0){                      //Checks if the Student is registered to a course
            System.out.println(Style.yellow("PLEASE CHOOSE THE STUDENT'S COURSE FIRST"));
            Course course = OtherViews.courseChoose.chooseOne(data.getCoursesFromStudent(student));
            if(data.getAssignmentsFromCourse(course).size()>0){
                ArrayList<Assignment> studentUnratedAssignments = data.getStudentUnratedAssignmentsFromCourse(student,course); // Takes all unrated student assignments from a course.
                if(studentUnratedAssignments.size()>0){
                    ...
                    if(sure == true){
                        StudentGrade grade = new StudentGrade(assignment,course,oralMark,totalMark);
                        data.saveStudentGrade(student, grade);
    ...
    public static void editStudentGrade(Student student){
        ...
        if(data.getCoursesFromStudent(student).size()> 0){                      //Checks if the Student is registered to a course 
            System.out.println(Style.yellow("PLEASE CHOOSE THE STUDENT'S COURSE FIRST"));
            Course course = OtherViews.courseChoose.chooseOne(data.getCoursesFromStudent(student)); //User Chooses a course
            if(data.getAssignmentsFromCourse(course).size()>0){                 //Checks if there are assignments in that course
                ArrayList<Assignment> studentRatedAssignments = data.getStudentRatedAssignmentsFromCourse(student,course); // Takes all rated student assignments from a course.
                if(studentRatedAssignments.size()>0){                           //Checks if there are Student's rated Assignments
                    ...
                    StudentGrade studentGrade = data.getStudentGradeForAssignment(student, course, assignment); //Data gives to the user the rated assignment
    ...
    private static void deleteStudent(Student student){
        ...
        if(result == true){
            ...
            ObjectMenu.showFor(DataBase.viewFor.EDIT);
        }
