...
public class EditAssignment {
    ...
    private static void chooseEditor(){
        ...
        Assignment chosenAssignment = OtherViews.assignmentChoose.chooseOne(data.getAssignments()); // Lets the user to choose one assignment
        Style.addLines(1);
    ...
    private static void addCourse(Assignment assignment){
        ...
        ArrayList<Course> coursesAreNotInAssignment = data.getCoursesAreNotInAssignment(assignment);//the database returs all courses that are not associated with that assignment
        ...
        if(coursesAreNotInAssignment.size()>0){                                 //checks if there are more courses to add in assignment.
            ...
            while(keepAsk == true){
                ...
                if(confirmedChanges == true){
                    data.addCourseInAssignment(course, assignment); //adds the course in the assignment
    ...
    private static void removeCourse(Assignment assignment){
        ...
        boolean keepAsk = true;
        while(keepAsk == true){
            if(0<data.getCoursesFromAssignment(assignment).size()){             //Checks if assignment's course list is no empty
                
                Style.addLines(1);
                Style.addUnderline();
                System.out.println(Style.yellow("LET'S REMOVE THE COURSE"));
                Course course = OtherViews.courseChoose.chooseOne(data.getCoursesFromAssignment(assignment));
                boolean confirmedChanges = OtherViews.confirmChanges();         //User confirms that wants to save the changes
                if(confirmedChanges == true){
                    data.removeCourseFromAssignment(course, assignment);        //removes the course from the assignment
                ...
                Style.addLines(1);
                if(data.getCoursesFromAssignment(assignment).size()>0){
