...
public class EditCourse {
    ...
    private static void chooseEditor(){
        ...
        Course chosenCourse = OtherViews.courseChoose.chooseOne(data.getCourses());// Lets the user to choose one course
    ...
    private static void addStudent(Course course){
        ...
        ArrayList<Student> studentsAreNotInCourse = data.getStudentsAreNotInCurse(course);//the database returs all students who are not associated with that course
        ...
        if(studentsAreNotInCourse.size()>0){                                    //checks if there are more students to add in course.
            ...
            while(keepAsk == true){
                ...
                if(confirmedChanges == true){
                    data.addStudentInCourse(student, course);                   //adds the student in the course
    ...
    private static void removeStudent(Course course){
        ...
        boolean keepAsk = true;
        while(keepAsk == true){
            if(0<data.getStudentsFromCourse(course).size()){                    //Checks if course's student list is no empty
                
                Style.addLines(1);
                Style.addUnderline();
                System.out.println(Style.yellow("LET'S REMOVE THE STUDENT"));
                Student student = OtherViews.studentChoose.chooseOne(data.getStudentsFromCourse(course));
                boolean confirmedChanges = OtherViews.confirmChanges();         //User confirms that wants to save the changes
                if(confirmedChanges == true){
                    data.removeStudentFromCourse(student, course);              //removes the student from the course
                ...
                Style.addLines(1);
                if(data.getStudentsFromCourse(course).size()>0){
    ...
    private static void addTrainer(Course course){
        ...
        ArrayList<Trainer> trainersAreNotInCourse = data.getTrainersAreNotInCurse(course);//the database returs all trainers who are not associated with that course
        ...
        if(trainersAreNotInCourse.size()>0){                                    //checks if there are more trainers to add in course.
            ...
            while(keepAsk == true){
                ...
                boolean confirmedChanges = OtherViews.confirmChanges();         //User confirms that wants to save the changes
                if(confirmedChanges == true){
                    data.addTrainerInCourse(trainer, course);       //adds the trainer in the course
    ...
    private static void removeTrainer(Course course){
        ...
        boolean keepAsk = true;
        while(keepAsk == true){
            if(0<data.getTrainersFromCourse(course).size()){                    //Checks if course's trainer list is no empty
                
                Style.addLines(1);
                Style.addUnderline();
                System.out.println(Style.yellow("LET'S REMOVE THE TRAINER"));
                Trainer trainer = OtherViews.trainerChoose.chooseOne(data.getTrainersFromCourse(course));
                boolean confirmedChanges = OtherViews.confirmChanges();         //User confirms that wants to save the changes
                if(confirmedChanges == true){
                    data.removeTrainerFromCourse(trainer, course);              //adds the trainer in the course
                ...
                Style.addLines(1);
                if(data.getTrainersFromCourse(course).size()>0){
